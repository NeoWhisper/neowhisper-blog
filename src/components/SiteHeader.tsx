    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-white/70 backdrop-blur-xl dark:bg-black/30">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-4 text-xs font-semibold uppercase tracking-[0.15em] text-gray-600 dark:text-gray-300 md:flex">
          {navItems.map((item) => (
            <Link
              ...
              className="rounded-full border border-white/20 bg-white/50 px-3 py-1 transition-all duration-300 hover:-translate-y-0.5 hover:bg-white dark:border-white/10 dark:bg-white/5"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex flex-wrap items-center gap-2 md:hidden">
          <Link
            href={withLang("/services", currentLang)}
            className="rounded-full border border-white/20 bg-white/60 px-3 py-1 text-xs font-semibold text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-200"
          >
            Services
          </Link>
          <Link
            href={withLang("/projects", currentLang)}
            className="rounded-full border border-white/20 bg-white/60 px-3 py-1 text-xs font-semibold text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-200"
          >
            Projects
          </Link>
          <Link
            href={withLang("/blog", currentLang)}
            className="rounded-full border border-white/20 bg-white/60 px-3 py-1 text-xs font-semibold text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-200"
          >
            Blog
          </Link>
          <Link
            href={withLang("/about", currentLang)}
            className="rounded-full border border-white/20 bg-white/60 px-3 py-1 text-xs font-semibold text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-200"
          >
            About
          </Link>
