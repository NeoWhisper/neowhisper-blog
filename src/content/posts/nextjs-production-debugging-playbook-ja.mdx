...
- [なぜ「ローカルOK / 本番NG」が起きるのか](#なぜ-ローカルok-本番ng-が起きるのか)
- [Step 1: まずはHTTPで障害範囲を測る](#step-1-まずはhttpで障害範囲を測る)
- [Step 2: ローカルでも必ず本番モードで再現確認](#step-2-ローカルでも必ず本番モードで再現確認)
- [Step 3: 失敗ルートではトップレベルimportを疑う](#step-3-失敗ルートではトップレベルimportを疑う)
- [Step 4: エラー処理中の「連鎖クラッシュ」を防ぐ](#step-4-エラー処理中の-連鎖クラッシュ-を防ぐ)
- [Step 5: デプロイ後スモークテストを固定化](#step-5-デプロイ後スモークテストを固定化)
- [チェックリスト](#チェックリスト)
